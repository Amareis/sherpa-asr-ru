...
function createOnlineRecognizer(Module, modelKey, sampleRate, decodingMethod) {
  // modelKey: 'vosk' или 't1'
  // Если не указан, используем 'vosk' по умолчанию
  modelKey = modelKey || 'vosk';
  // sampleRate: 16000 для Vosk, 8000 для T-One
  sampleRate = sampleRate || 16000;
  // decodingMethod: 'greedy_search' или 'modified_beam_search'
  ...
  // Настраиваем конфигурацию в зависимости от выбранной модели
  console.log(`[CONFIG] Creating recognizer for model: ${modelKey}, sampleRate: ${sampleRate}`);
  
  if (modelKey === 'vosk') {
    // Vosk Zipformer transducer (int8)
    onlineTransducerModelConfig.encoder = 'encoder.int8.onnx';
    onlineTransducerModelConfig.decoder = 'decoder.onnx';
    onlineTransducerModelConfig.joiner = 'joiner.int8.onnx';
    console.log('[CONFIG] Using Vosk transducer model (16kHz)');
  } else if (modelKey === 't1') {
    // T-One CTC model
    onlineToneCtcModelConfig.model = 'model.onnx';
    console.log('[CONFIG] Using T-One CTC model (8kHz)');
  } else {
    console.warn(`[CONFIG] Unknown model key: ${modelKey}, defaulting to vosk`);
    onlineTransducerModelConfig.encoder = 'encoder.int8.onnx';
    onlineTransducerModelConfig.decoder = 'decoder.onnx';
    onlineTransducerModelConfig.joiner = 'joiner.int8.onnx';
  }
  ...
  const onlineModelConfig = {
    ...
    debug: 1,
    modelType: '',
    modelingUnit: '',  // Пустая строка для русского языка
    bpeVocab: '',
  };

  // Используем правильный sample rate для модели
  // T-One использует 8kHz, Vosk использует 16kHz
  const featureConfig = {
    sampleRate: sampleRate,
    featureDim: 80,
  };

  console.log(`[CONFIG] Feature config: sampleRate=${featureConfig.sampleRate}, featureDim=${featureConfig.featureDim}`);
  console.log(`[CONFIG] Decoding method: ${decodingMethod}`);

  let recognizerConfig = {
    featConfig: featureConfig,
    modelConfig: onlineModelConfig,
    decodingMethod: decodingMethod,
    maxActivePaths: 4,  // используется для modified_beam_search
    enableEndpoint: 1,
