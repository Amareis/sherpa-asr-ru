...
2. Дождитесь загрузки модели (первый раз ~30 сек для Vosk, ~2 мин для T-One)
3. Нажмите "Start" и говорите в микрофон
4. Текст появится в реальном времени

**Смена модели:**
- Выберите другую модель в radio buttons
- Нажмите "Load Model" - страница перезагрузится с новой моделью
- Модели кэшируются, повторная загрузка ~1 сек
...
### FST языковая модель для T-One

T-One - character-level CTC модель без языковой модели, поэтому может делать орфографические ошибки. Для улучшения качества можно добавить FST (Finite State Transducer) языковую модель:

- T-One поставляется с 5-gram LM в формате FST (~5.46GB)
- sherpa-onnx поддерживает CTC decoding с FST через `ctc_fst_decoder_config`
- Это значительно улучшит орфографию, но увеличит размер и время загрузки

```javascript
// Пример конфигурации с FST (не реализовано)
ctcFstDecoderConfig: {
  graph: 'HLG.fst',  // FST граф
  maxActive: 3000
}
```
